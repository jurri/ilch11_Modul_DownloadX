...
switch ( $menu->get(1) ) {
  default :
    ...
    if ( db_num_rows($erg) > 0 ) {
		  $tpl->out(1); $class = 'Cnorm';
			...
			while ($row = db_fetch_assoc($erg) ) {
				$files = count_files($row['id']);
				$class = ( $class == 'Cmite' ? 'Cnorm' : 'Cmite' );
				/*if($rowid%2 == 1){
					$row['ausgabe_all'] = "<tr class=\"".$class."\">";
					$row['ausgabe_all'] .= "<td><a href=\"index.php?downloads-".$row['id']."\"><img src=\"/include/images/downloads/".$row['id'].".png\" width=\"350\" height=\"134\"/></a><br /><span class=\"smalfont\">".$row['desc']."</span></td><td align=\"center\">".$files."</td>";
					$row['ausgabe_all'] .= "<td bgcolor=\"#C531B5\">&nbsp;</td>";
				}else{
					$row['ausgabe_all'] .= "<td><a href=\"index.php?downloads-".$row['id']."\"><img src=\"/include/images/downloads/".$row['id'].".png\" width=\"350\" height=\"134\"/></a><br /><span class=\"smalfont\">".$row['desc']."</span></td><td align=\"center\">".$files."</td></tr>";
				}*/
				if($row['id']%2 == 1){
					$row['ausgabe_all'] = "<tr class=\"".$class."\">";
					$row['ausgabe_all'] .= "<td><a href=\"index.php?downloads-".$row['id']."\"><img src=\"/include/images/downloads/".$row['id'].".png\" border=\"0\" width=\"350\" height=\"134\"/></a><br /><span class=\"smalfont\">&raquo;&nbsp;".$row['desc']."</span></td><td align=\"center\">".$files."</td>";
					$row['ausgabe_all'] .= "<td bgcolor=\"#C531B5\">&nbsp;</td>";
					
					$rowid = $row['id'];
					$rowid++;
					$files = count_files($rowid);
					$row1 = db_fetch_assoc(db_query("SELECT `desc` FROM prefix_downcats WHERE id = $rowid"));	
					
					$row['ausgabe_all'] .= "<td><a href=\"index.php?downloads-".$rowid."\"><img src=\"/include/images/downloads/".$rowid.".png\" border=\"0\" width=\"350\" height=\"134\"/></a><br /><span class=\"smalfont\">&raquo;&nbsp;".$row1['desc']."</span></td><td align=\"center\">".$files."</td></tr>";
				}else{
					$row['ausgabe_all'] = '';
				}
    ...
    $tpl->set ('DAM', $DAM);
		...
		$erg = db_query("select id,name,version,ssurl,surl,`desc`,downs,DATE_FORMAT(time,'%d.%m.%Y') as datum from prefix_downloads WHERE cat = ".$cid." ORDER BY ".$sortierung);
		if ( db_num_rows($erg) > 0 ) {
        ...
        # smal screenshot url
				...
				$row['vorschau'] = '<img src="'.$row['surl'].'" alt="'.$row['name'].' '.$row['version'].'" title="'.$row['name'].' '.$row['version'].'" border: none; padding-right:5px;" width="50px" height="50px" />';
				$row['thumbslide'] = $row['surl'];
		$class = ( $class == 'Cmite' ? 'Cnorm' : 'Cmite' );
				$row['class'] = $class;
